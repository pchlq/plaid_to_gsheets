...
start_date = "{:%Y-%m-%d}".format(
    datetime.datetime.now() + datetime.timedelta(-PERIOD_DAYS)
)
...
if __name__ == "__main__":
    ...
    (
        pd.DataFrame.from_dict(transactions_response["transactions"]).to_csv(
            f"raw_tansactions{PERIOD_DAYS}_{TODAY}.csv", index=False
        )
    )
